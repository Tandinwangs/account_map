        ...
        <div class="row" id="individual_row" style="display: none;">
          ...
          <div class="col-md-6">
            <div class="form-group first">
              ...
              id="phone_number" pattern="(17|77)\d{6}" title="Please enter a valid 8-digit number starting with 17 or 77.">
...
<!-- <script>
  $(document).ready(function() {
    ...
    $('#customer_type').on('change', function() {
      ...
      if (selectedOption === 'individual') {
        ...
        $('#phone_number').on('input', function() {
          ...
          console.log('Old nu', phoneNumber);
          if (accountNumber && enteredCID && phoneNumber) {
            fetchAccountDetails(accountNumber, enteredCID, phoneNumber);
          }
        });
        });
        ...
        $('#corporate_row').show();
        resetForm();
        // Add input event listener for old account number and phone
        ...
        $('#phone').on('input', function() {
          ...
          // Check if old account number is also provided
    ...
    function fetchAccountDetails(accountNumber, enteredCID, phoneNumber) {
      fetch('/check-account-number/' + accountNumber)
        ...
        .then(function(data) {
          if (data.exists) {
            // Account number exists, now check CID and phone number
            if (data.exists.cid === enteredCID && data.exists.mobile === phoneNumber) {
              $('#preloader').show();
              setTimeout(function() {
                $('#preloader').hide();
                $('#dataContainer').show();
              ...
              console.log('Entered CID or phone number does not match the stored values');
            }
          } else {
    ...
    function fetchAccountDetails1(oldAccountNumber, enteredPhone) {
      fetch('/check-account-number-phone/' + oldAccountNumber)
        ...
        .then(function(data) {
          ...
          if (data.exists) {
            // Account number exists, now check phone number
